...
namespace Ejercicio.Persistence
{
       ...
       where T : class
    {
        ...
        public async Task<IEnumerable<T>> GetAllAsync(int take, int skip = 0, string orderBy = "", string whereSql = "", IDbTransaction transaction = null, bool mostrarBorrados = false)
        {
            return await this.GetAllCustomAsync<T>(take, skip, null, "", whereSql, orderBy, transaction, mostrarBorrados);            
        }

        public async Task<IEnumerable<E>> GetAllCustomAsync<E>(int take, int skip = 0, string propSelectSql = null, string joinsSql = "", string whereSql = "", string orderBy = "", IDbTransaction transaction = null, bool mostrarBorrados = false)
        {
            ...
            return await QueryAsync<E>(query, transaction);
        }
        ...
        public async Task<bool> ExistsAsync(Guid Uid, bool mostrarBorrados = false) {
            bool existe = false;
            if (nombrePropiedadUid == null)
            {
                return existe;
            }
            string query = string.Format(ConsultasSQL.SQL_EXISTS_BY_Uid, this.nombreTabla, nombrePropiedadUid);
            if (!mostrarBorrados && softDelete)
            {
                query += ConsultasSQL.SQL_FILTRO_FECHA_BORRADO;
            }
            var connection = this.GetConnection(null);

            try
            {
                int resultado = await connection.ExecuteScalarAsync<int>(query, new { Uid = Uid });
                existe = resultado > 0;
            }
            catch (Exception ex)
            {
            ...
            {
                DisposeConnection(connection, null);
            }
        ...
        public async Task<T> GetByUidAsync(Guid Uid, IDbTransaction transaction = null, bool mostrarBorrados = false)
        {
            T entidad = null;
            if (nombrePropiedadUid == null)
            {
            ...
            try
            {
                entidad = await connection.QuerySingleOrDefaultAsync<T>(query, new { Uid = Uid }, transaction);
            }
            catch (Exception ex)
            {
                trazaLoggerInterceptor.GuardarExcepcion(
        ...
        public async Task<bool> InsertAsync(T entidad, IDbTransaction transaction = null)
        {
            string propiedades = string.Empty;
            string propiedadesInsert = string.Empty;
            ...
            return await InsertAsync(entidad, transaction, propiedadesTablaInfoInsert, propiedades, propiedadesInsert, propertyInfoId, propiedadInfoUid);
        }
        public async Task<bool> InsertAsync<E>(E entidad, IDbTransaction transaction = null)
        {
            ...
            return await InsertAsync<E>(entidad, transaction, propiedadesE.ToList(), propiedades, propiedadesInsert, proInfoId, proInfoUid);
        }
        ...
        private async Task<bool> InsertAsync<E>(E entidad, IDbTransaction transaction, List<PropertyInfo> propiedadesInsertInfo, string propiedades, string propiedadesInsert, PropertyInfo proInfoId = null, PropertyInfo proInfoUid = null)
        {
            if (entidad == null)
            {
                return false;
            ...
            try
            {
                ...
                var respuestaId = (await connection.QueryAsync(query, param: entidad, transaction: transaction)).SingleOrDefault();
                if (proInfoId != null)
                {
        ...
        public async Task<bool> UpdateAsync(T entidad, IDbTransaction transaction = null, bool mostrarBorrados = false)
        {
            var propiedadesSet = string.Join(",", propiedadesTablaUpdate.Select(p => string.Format(ConsultasSQL.SQL_SET, p)));

            return await UpdateAsync<T>(entidad, transaction, propiedadesTablaInfoUpdate, propiedadesSet, mostrarBorrados);
        }
        ...
        private async Task<bool> UpdateAsync<E>(E entidad, IDbTransaction transaction, List<PropertyInfo> propiedadesInsertInfo, string propiedadesSet, bool mostrarBorrados = false)
        {
            if (entidad == null)
            {
                return false;
            ...
            try
            {
                ...
                entidadesActualizadas = await connection.ExecuteAsync(query, param: entidad, transaction: transaction);
            }
            catch (ValidationDBException vex)
            {
                throw vex;
        ...
        public async Task<bool> SoftDeleteAsync(Guid Uid, IDbTransaction transaction = null)
        {
            if (!softDelete)
            {
                return false;
            ...
            try
            {
                elementosEliminados = await connection.ExecuteAsync(query, new { Uid = Uid });
            }
            catch (Exception)
            {
                transaction?.RollbackAndCloseConnection();
        ...
        public async Task<bool> DeleteByUidAsync(Guid Uid, IDbTransaction transaction = null) {
            if (nombrePropiedadUid == null)
            {
                return false;
            }
            ...
            try
            {
                elementosEliminados = await connection.ExecuteAsync(query, new { Uid = Uid }, transaction: transaction);
            }
            catch (Exception ex)
            {
                trazaLoggerInterceptor.GuardarExcepcion(
